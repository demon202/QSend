 ...
 * app.js
 *
 * Root cause of "stuck at peer-joined / joined session" bug:
 *
 *   state.pc was created AFTER connectSignaling() resolved.
 *   On fast networks, 'peer-joined' could arrive and call
 *   createAndSendOffer() while state.pc was still null —
 *   crashing silently with "Cannot read properties of null".
 *
 * Fix: For the sender, state.pc and state.dc are created
 * BEFORE the WebSocket message is sent to the server.
 * This guarantees state.pc is never null when peer-joined fires.
 * The offer is still only sent after peer-joined (so the receiver
 * is in the session), but now state.pc always exists.
 */
...
function setupDataChannel(dc) {
  console.log('[DC] Setting up data channel, readyState:', dc.readyState);
  dc.binaryType = 'arraybuffer';
  dc.bufferedAmountLowThreshold = CONFIG.RESUME_BUFFER;

  dc.onopen = async () => {
    console.log('[DC] Open — starting ECDH key exchange');
  ...
  dc.onmessage = async ({ data }) => {
    if (typeof data === 'string') {
      ...
      console.log('[DC] Control message:', msg.type);
...
async function onChunk(buf) {
  ...
  state.chunks[index] = data; state.rcvdChunks++;
  state.bytesDone += data.length; updateSpeed(data.length);
...
/**
 * THE FIX: For the sender, state.pc and state.dc are created
 * BEFORE ws.send({ type:'create' }) — so state.pc is always
 * non-null when the 'peer-joined' message arrives.
 *
 * Previous versions created state.pc after connectSignaling()
 * resolved, which was a race: 'peer-joined' could arrive in the
 * same microtask queue flush and call state.pc.createOffer()
 * while state.pc was still null.
 */
function connectSignaling(joinCode) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ws.close(); reject(new Error('Signaling server timed out'));
    }, 10_000);
    ...
    ws.onopen = () => {
      ...
      if (state.mode === 'send') {
        ...
        // 'peer-joined' arriving before state.pc is assigned.
        console.log('[SEND] Creating PC and DataChannel before sending create');
    ...
    ws.onmessage = async ({ data }) => {
      ...
      console.log('[WS] Received:', msg.type, msg.code || msg.message || '');
      ...
      switch (msg.type) {
        ...
        case 'joined':
          // Receiver side: resolved, waiting for offer
          UI.status('Joined session — waiting for sender…', 'info');
          resolve();
          break;

        case 'peer-joined':
          // Sender side: receiver has joined, safe to send offer now.
          // state.pc is guaranteed non-null (created in ws.onopen above).
          console.log('[SEND] peer-joined received, state.pc:', state.pc ? 'exists' : 'NULL ← BUG');
          UI.status('Peer connected — negotiating connection…', 'info');
          try {
            const offer = await state.pc.createOffer();
            await state.pc.setLocalDescription(offer);
            console.log('[SEND] Offer created, sending to receiver');
        ...
        case 'offer': {
          // Receiver side
          console.log('[RECV] Received offer, creating PC');
          state.pc = createPeerConnection();
          state.pc.ondatachannel = ({ channel }) => {
            console.log('[RECV] Got data channel:', channel.label);
            state.dc = channel;
            setupDataChannel(channel);
          };

          await state.pc.setRemoteDescription(new RTCSessionDescription(msg.sdp));
          console.log('[RECV] Remote description set');

          // Drain ICE buffer — must be AFTER setRemoteDescription
          ...
          console.log('[RECV] Answer created, sending');
        ...
        case 'answer':
          // Sender side
          console.log('[SEND] Received answer, setting remote description');
          await state.pc.setRemoteDescription(new RTCSessionDescription(msg.sdp));
          ...
          state.pendingICE = [];
          console.log('[SEND] Remote description set, ICE negotiation underway');
          break;
          ...
          if (msg.candidate) {
            if (state.pc && state.pc.remoteDescription) {
              try {
                ...
                console.log('[ICE] Applied candidate:', msg.candidate.type);
              } catch (e) { console.warn('[ICE] addIceCandidate failed:', e.message); }
            } else {
              console.log('[ICE] Buffering candidate (no remote desc yet)');
        ...
        case 'error':
          console.error('[WS] Signaling error:', msg.message);
...
function updateSpeed(bytes) {
  ...
  if (dt >= 500) {
    ...
    state.lastSpeedTs = now; state.lastSpeedBytes = 0;
  }
}
...
const UI = {
  ...
  setConnDot(s) {
    ...
    dot.className = 'conn-dot ' + (
      s==='connected'||s==='completed' ? 'green' :
      s==='checking' ||s==='new'       ? 'yellow': 'red');
  },
  setLockState(locked) {
...
async function initSend() {
  state.mode = 'send';
  // PC is created inside connectSignaling → ws.onopen, before any message is sent.
  // See the comment block in connectSignaling() for the full explanation.
...
function initDrop() {
  ...
  document.addEventListener('drop', e => {
    e.preventDefault();
    if (e.dataTransfer.files.length && state.mode !== 'receive') {
      Array.from(e.dataTransfer.files).forEach(enqueueFile);
      showFileSelected(e.dataTransfer.files[0]);
    }
  });
...
async function init() {
  ...
  document.getElementById('reset-btn')?.addEventListener('click', () => {
    state.reset(); location.href = location.pathname;
  });
